...
跑完 5 篇（每篇都走完 `/yang-predict` → 发布 → `/yang-retro` 闭环）之后，你会有第一份个人校准数据，可以提议第一次 `/yang-bump` 升级到 v1（或直接采用 [opinion-video.md](opinion-video.md) 的 v2 作为起点重新校准权重）。
...
```
composite = (ER + HP + QL + NA + AB + SR + SAT) / 7
```
...
如果你的频道走真诚路线，SAT 给 6 当占位即可。

---

## Bucket 预测：所有阶段统一格式 + 渐进信心标注

> 早期预测精度差是数学事实——**不通过省略 bucket 解决**，通过 **header 的 confidence 等级 + 概率分布更平**来诚实表达不确定。详见 [shared-protocols/prediction-anatomy.md](../shared-protocols/prediction-anatomy.md) 和 [state-management.md 的 confidence 表](../shared-protocols/state-management.md)。
>
...
跑完 5 篇后，你的 baseline 已经从 5 个数据点稳定。`/yang-status` 会主动提示：

> 你已校准 5 篇，可以基于实际分布固定绝对 bucket 边界。
> 跑 `/yang-bump --bucket-only` 自动派生。
...
- 第 5 篇之后**必须**跑 `/yang-bump --bucket-only` 重新校准
...
3. **记录锚点假设**：写下"我押 ER=10 比 ER=6 流量高"这样的假设，复盘验证 → rubric 升级的证据

**你不应该做的事**：基于 cold-start 期的 composite 决定要不要发某条稿子。**精度 ±50%——决策没意义**。已经决定要发的，跑完整循环；正在犹豫要不要发的，靠你自己感觉。

### 前 5 篇的取样策略：**主动选差异最大的稿子**

如果你前 5 篇都是 `ER=10 / SR=4 / HP=10` 的同款，复盘时**你无法**告诉哪个维度真的预测了流量——多重共线性。
...
### 路径 A：从 v0 → v1（自己拟合权重）
跑 `/yang-bump --propose "<你的具体权重调整>"`。系统会强制全量重打 + 跨模型审核。

### 路径 B：直接采纳 v2 作为起点
把 [opinion-video.md](opinion-video.md) 的 v2 公式抄到你的 `rubric_notes.md`，**然后跑 `/yang-bump`**——bump 流程会用你的 5 篇数据验证 v2 在你账号上是否真的更优。
