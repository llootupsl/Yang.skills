...
```
composite = (ER×1.5 + SR×1.5 + HP×1.5 + QL + NA + AB + SAT) / 17.0 × 2.0
```
...
- **0** — 纯信息传递；没有情感钩子
- **6** — 一般共鸣（"嗯，我也有过这种感觉"）
- **10** — 锐利、具体、**让人不太愿意承认**的自我识别。观众心想"这就是我，我之前不想承认"

10/10 anchor：暗恋自欺、家庭义务、内化的失败感——任何让观众认出自己同时微微感到不适的内容。
0 分对照：纯学术陈述、抽象概念论文。
...
- **0** — 纯个人 / 人际层面
- **6** — 触到一个公认的社会现象（职场动态 / 恋爱 / 家庭），但没加新视角
- **10** — 命名了一个观众**认识但没有语言形容**的结构性模式

10/10 anchor：参考博主的"房价/状元"——"学生坠楼 vs 高考状元"的非对称冲击直击阶层 / 教育 / 生命价值三个议题交叉。
0 分对照：纯个人情感（暗恋 / 关系）。
...
- **0** — 通用开场（"大家好，今天聊一聊..."）
- **6** — 具体的承诺或反直觉断言
- **10** — 一个具体生动的场景或断言，观众无法停止处理

不要把 HP 与"猎奇"混淆。一个安静但具体的开场（"有些关系不是在某一刻结束的——它们在你还在里面的时候就已经偏移"）也可以是 HP=10。
...
- **0** — 全是叙述；没有警句
- **6** — 结尾有一句令人记住的
- **10** — 多句独立可用、分布在稿子不同位置（开场 / 中间 / 结尾）

> **校准 caveat**：参考博主的数据显示 **QL=10 不在于"金句数量"**，更接近"每句金句离开视频后还能在评论里 / 截图里 / 朋友的微信里独立存活吗？"——这个细微的定义偏差会被未来的 v2.1 升级吸收。
...
- **0** — 真诚直陈
- **6** — 一层反讽
- **10** — 嵌套或自指反讽（例如，一篇**关于**某现象的稿子，**用那个现象的格式**写出来）

如果你的频道走真诚路线，SAT 给 6 当占位，**不要**让它影响排序。如果你的频道走戏仿路线，这个维度重要。

10/10 anchor：参考博主的"房价/状元"——三层反讽（虚构国框架 + 学术论文形式戏仿 + 致谢段自我指涉）。
...
> **如果你想试 v2.1**：跑完 5 篇 v2 校准 + 主动收集 MS 和 TS 的"试评分"作为候选维度数据。证据足够（≥3 样本支持）后通过 `/yang-bump` 走完整升级流程。
...
> Calibration 用户（≥5 篇有数据）由 `/yang-init` 询问 baseline 后自动派生。
...
校准过 ≥5 篇后跑 `/yang-bump --bucket-only` 让系统基于你的实际分布自动派生。
...
## 与 `Yang.skills` 子 skill 的对应

这份 starter rubric 被以下 skill 读取：
- `/yang-init` cold-start 模式 → 默认复制本文件到 `rubric_notes.md`（如选择 calibration 模式且 mode 决定能用 v2 公式）
- `/yang-score` 和 `/yang-predict` → 读 `rubric_notes.md` 拿当前公式
- `/yang-bump` → 这份 starter 是 bump 的"先验起点"，bump 后被替换为用户自己拟合的版本
