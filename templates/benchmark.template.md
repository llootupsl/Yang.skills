...
>
> 你随时可以：
> - **追加**对标视频：`/yang-learn-from --append <new-videos>`
> - **替换**对标账号：`/yang-learn-from --replace <new-account>`
...
> yang-seed brainstorm 时会读这里——根据对标账号的主题分布给提议。
...
> yang-seed Mode A/B/C 仍以你的实际想做什么为主。
...
- 当 `state.calibration_samples >= 10` 时，yang-status 会提示"你已有足够自己数据，benchmark 主要作 sanity check 不再主导"
- 但 benchmark.md **不删**——它仍是 yang-seed brainstorm 的 reference frame
- 如果对标账号本身风格变了 / 你不想再对标了 → 跑 `/yang-learn-from --replace <none>` 解除（保留历史在文件，但 yang-seed 不再读）
