...
>
> 完整生命周期协议见 [shared-protocols/observation-lifecycle.md](../Yang.skills/shared-protocols/observation-lifecycle.md)。
> 升级流程见 [shared-protocols/bump-validation-protocol.md](../Yang.skills/shared-protocols/bump-validation-protocol.md)。
...
> Cold-start 用户应该等权起步——见 [opinion-video-zero.md](../Yang.skills/starter-rubrics/opinion-video-zero.md)。
...
> ```
>
> 删除规则见 [shared-protocols/observation-lifecycle.md](../Yang.skills/shared-protocols/observation-lifecycle.md)：被吸收为维度 → 删；被推翻 → 删。git history 是档案。

### 示例条目（参考格式模板；你的项目的真实条目从复盘后开始累计）
...
>
> 本工具按校准阶段切换三种 bucket 方案。当前生效方案由 `.yang-state.json` 的 `bucket_scheme` 字段决定。
...
详见 [starter-rubrics/opinion-video-zero.md](../Yang.skills/starter-rubrics/opinion-video-zero.md) 的"比率桶方案"段。

### 阶段 2：N=5 后切到固定绝对桶（带 ratio 备用）

`bucket_scheme = "absolute_with_ratio"`

跑完 5 篇后，`/yang-bump --bucket-only` 自动派生：
- `baseline = 5 篇实际播放的中位数`
- 边界 = baseline × {0.3 / 1 / 3 / 10 / 30}

`/yang-bump --bucket-only` 落地时会替换本段表格。
...
`/yang-status` 在 N=10 时主动建议切换。这种方案永远自洽——不管账号多大，"top 5%"语义稳定。
