...
> ⚠️ **本文件是 Channel A 内部参考。Channel B（yang-score-blind sub-agent）**永远不读**本文件**——blind sub-agent 的 hard refusal list 显式包含本文件路径。
>
...
yang-bump Phase 5 落地时把升级 Memo 写**这里**，**不写**进 `rubric_notes.md`。

历史上 yang-bump 把 Memo 全文（含视频名 + 实绩 + 派生证据）写到 `rubric_notes.md`。但 `rubric_notes.md` 是 blind sub-agent 的白名单文件——sub-agent 通过它泄漏实绩数据，本应隔离的盲打分变成"看过实绩的事后解释"。
...
## 写入规则（yang-bump Phase 5）

- **追加模式**：新 bump 的 Memo **append 到文件末尾**，旧 Memo 不动。按时间倒序倒着读可看 rubric 演化全程
- **每段 Memo 必含 6 个组件**（按 [bump-validation-protocol.md](../shared-protocols/bump-validation-protocol.md) Step 5 模板）：
...
| Skill | 读？ | 干啥 |
|---|---|---|
| `yang-bump` Phase 5 | ✅ 写 | append 新 Memo |
| `yang-retro` | ✅ 读 | 复盘时回看历史 Memo 找 rubric 演化轨迹 |
| `yang-status` | ✅ 读 | 看板上显示"上次 bump 用了什么证据" |
| **`yang-score-blind`** | ❌ **硬禁** | refusal_code: `blocked_rubric_memo` |
| `yang-score` / `yang-predict` 主 Claude 自估那部分 | 不主动读 | 主对话本身已被污染，再读一遍 Memo 也只是冗余 |

---

## Memo 段格式（yang-bump 用此格式写）
...
（yang-init 创建时本文件为空，首次 bump 时在此追加第一段 Memo）
