...
找选题
  ...
  └─ 发过历史的    → /yang-seed brainstorm（兴趣 × 热点 × 你过去做过什么）
                    （都跑 yang-seed，区别是发过的 brainstorm 时多一份历史 context）
  ↓
yang-seed 写 5 个草稿到 → scripts/<日期>_<id>_<short>.md
  ↓
用户改写 scripts/<日期>_<id>_<short>.md（同一文件覆盖）
  ↓
/yang-score scripts/<日期>_<id>_<short>.md → 看 rubric 评分（探索）
  ↓
/yang-predict scripts/<日期>_<id>_<short>.md → 写 immutable 预测 v1 到 predictions/
  ↓
拍摄完 → /yang-shoot scripts/<日期>_<id>_<short>.md
   ├─ 建 videos/<日期>_<id>_<short>/ 目录
   ├─ 询问用户："拍时实际用的稿子和 scripts/<id>.md 一致吗？"
   │   ├─ 一致 → cp → videos/<id>/script.md，沿用 v1 预测
   │   ├─ 改了 → 要最终稿 → 算 diff
   │   │   ├─ diff ≥30% → 自动 /yang-predict — mode: v2 → predictions/<id>.md append `## 预测 v2` 段
   │   │   └─ diff <30% → 询问是否 v2，默认沿用 v1
   │   └─ 大改 → 走 _redo 流程（新 scripts/<id>_redo.md + 重 yang-predict）
   └─ buffer +1
  ↓
发布 → /yang-publish + URL → buffer -1
  ↓
T+3 天 → /yang-retro videos/<日期>_<id>_<short>/
...
> **v2 重判触发**：yang-shoot 检测拍摄稿与原 scripts 的 line-diff ≥30% 时自动调用 yang-predict 写 `## 预测 v2` 段（append，不覆盖 v1）。详见 [shared-protocols/prediction-anatomy.md](../shared-protocols/prediction-anatomy.md) 的 v1/v2 段约定。
...
> 配了 adapter 的可以让 yang-retro 自动抓。
...
bump 是高风险操作——会做 5 步验证（含跨模型独立审）。详见 `Yang.skills/shared-protocols/bump-validation-protocol.md`。
...
├── candidates.md            # 候选池（可选；yang-seed / yang-trends 写）
├── .yang-state.json        # 状态文件（git track）
├── .yang-cache/            # 本地缓存（gitignore）
│   ├── usage.jsonl
│   └── trends-history.jsonl
├── .yang-secrets.json      # API key / cookie（gitignore）
├── .yang-hooks/            # hook 脚本副本
│   ├── prediction-immutability.sh
│   ├── session-start.sh
│   └── log-event.sh
├── .claude/settings.json    # 含 Yang.skills hooks
│
├── scripts/                 # **拍前的所有草稿**
│   └── YYYY-MM-DD_<id>_<short>.md   # yang-seed 写或用户写
│
├── predictions/             # **immutable 预测日志**（hook 保护）
│   └── YYYY-MM-DD_<id>_<short>.md   # yang-predict 写
│
...
| 目录 | 阶段 | 内容 | 谁写 |
|---|---|---|---|
| `scripts/` | 拍前草稿 | Claude AI 草稿或用户原创 | yang-seed 写初版；用户改写也在原文件 |
| `predictions/` | 预测锁定 | 7 组件 immutable 日志 | yang-predict 写 |
| `videos/<id>/` | 拍后产物 | 最终拍摄稿 + T+3d 数据 | yang-shoot 建目录；yang-retro 写 report.md |

三处用同一组 `<date>_<id>_<short>` 命名，`<id>` 是 `scripts/<id>.md` 首次落盘内容的 sha256 前 12 位，**草稿改写不变**。

`/yang-init` 自动创建以上骨架（不覆盖已存在的）。

---

## 卡住了？

- 看 `Yang.skills/SKILL.md` 的"必须拒绝的请求"段——你想做的事可能正好是被设计拒绝的
- 看对应子 skill 的 `Yang.skills/skills/yang-X/SKILL.md`
- 跑 "状态" 看 yang-status 的"下一步建议"
