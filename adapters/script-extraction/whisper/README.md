...
被 `/yang-learn-from` 在 Way b（用户提供视频文件，让工具转录）时调用。
...
`/yang-learn-from` 暂不直接支持云端 API——如果你有 OpenAI / Azure / 阿里云的 ASR API key，可以自己改 `run.sh` 走云端。

---

## 用法

yang-learn-from 自动调用，你不需要手动跑。但如果想手动测试：
...
├── README.md           # 本文件
└── run.sh              # yang-learn-from 调用的 wrapper
```

## 与其他 adapter 的关系

- 同 `adapters/perf-data/douyin-session/`、`adapters/trend-sources/*` 一样，是 Yang.skills 的可选 adapter
- 只在 `/yang-learn-from --way b` 时调用——Way a（粘文本）不需要
...
- **B站**：BBDown / you-get 等下载工具
- **YouTube**：yt-dlp 等下载工具
- **小红书**：XHS-Downloader 等下载工具

下载后扔到 `samples/<benchmark-name>/<video-id>/source.mp4` 即可——yang-learn-from 会自动找到。
