...
被以下 skill 调用：`yang-seed` Phase 2a、`yang-trends`。

> **当前状态**：已由 `fetch_trends.py` 实现可直接运行的抓取（零配置，公开端点+浏览器UA），并接入时效锁。本文件保留端点与字段映射细节，供单独微调解析时参考。

---

## 适用场景

- **`yang-seed` 默认 source 之一**——cold-start 用户的第一次选题种子
- **`yang-trends` 可选 source**——日常补充候选池
...
返回符合 [shared-protocols/candidate-schema.md](../../shared-protocols/candidate-schema.md) 的 items 列表。
...
- 其他字段：null（抓取阶段不打分；由调用方 yang-score 处理）
...
**优雅降级**：单次失败不抛异常——调用方（yang-seed / yang-trends）会用其他 sources 兜底。

## 稳定性等级

★★★ — 公开端点，但微博偶尔调整页面结构 + 有反爬（同一 IP 短时间高频抓取会被限流）。

建议节流：`/yang-seed` 默认每用户每天 ≤ 3 次抓取——cold-start 阶段不需要更高频。

## WebFetch 兜底（聚合器全部源不可用时）

当 `fetch_trends.py` 的全部源都不可用时，由 Claude 用 `WebFetch` 兜底抓 `https://s.weibo.com/top/summary?cate=realtimehot`，从 HTML 中提取 top 50 热搜标题：
...
- 微博热搜内容**经常含政治敏感 / 娱乐八卦**议题——/yang-seed Phase 1 Q3 的"红线"过滤至关重要
