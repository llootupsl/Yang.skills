...
## yang-seed / yang-trends 怎么调

**不要直接 curl** —— 让 Claude 自然触发 aihot skill 即可：

| yang-seed 场景 | 给 Claude 的内部指令 |
|---|---|
...
aihot skill 的 SKILL.md 已经详尽描述了端点 + 路由优先级（默认走精选不走日报）——yang-seed 不需要重复写这套逻辑，**信任 aihot skill 自己的判断**。

## 输出格式契约

aihot skill 默认返回 markdown，按 5 类（模型/产品/行业/论文/技巧）分组。yang-seed 收到后：

1. 按 `content_form` 过滤掉不相关类别（如 opinion-video → 留行业 + 产品；tutorial-builder → 留模型 + 工具）
2. 用当前 rubric 粗筛 5 条最适合的
3. 转成 [candidate-schema.md](../../shared-protocols/candidate-schema.md) 的 schema 写入 `candidates.md`
...
| 用户的 content_form 跟 AI 完全无关（如美食/妆教） | yang-seed 应该**不调 aihot**——按 [data-source-routing.md](../../shared-protocols/data-source-routing.md) 的路由表 |
