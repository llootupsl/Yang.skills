...
被 `/yang-retro` 在 `state.data_collection=adapter` + `platform=douyin` 时自动调用。

> **状态**：已通过 25+ 中文观点视频博主的实际生产验证。
...
输出写到 `<video_folder>/report.md`（yang-retro 读这个文件 → 摘要写到 prediction 复盘段）。
...
# ADAPTER="$HOME/Desktop/yang-test/adapters/perf-data/douyin-session"
...
yang-retro 自动调用，你不需要手动跑。但如果想手动测试：
...
- `https://v.douyin.com/abc123` → 短链需要 resolve（yang-publish 自动做）

yang-publish 会在登记发布时把 aweme_id 存到 prediction header（如能 resolve）。yang-retro 启动时直接读这个字段。
...
- `xhs-explore`（待）— 小红书也走 Playwright 路线，可复用本 adapter 的浏览器自动化方案
