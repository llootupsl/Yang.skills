...
被 `yang-migrate` skill / `yang-init` / SessionStart hook / 维护者引用。规定如何安全演进 `.yang-state.json` schema 而不让老用户被打断。
...
### 1. 改 yang-init 写新 state 的硬编码 schema_version
...
位置：`skills/yang-init/SKILL.md` Phase 3 的 state 写入段。
...
- WHAT changed
- WHY
- HOW (Claude steps for /yang-migrate)
- Manual fallback
...
if [[ "$state_schema" != "$LATEST_SCHEMA" ]]; then
  ...
  echo "   建议跑 /yang-migrate 升级（不阻塞继续工作）"
fi
```
...
`/yang-migrate` 写之前会备份到 `.yang-state.json.backup-<timestamp>`。

备份保留多久：
- 用户跑 `/yang-status` 时，如果有备份 + state 已稳定运行 N 天 → 提示"可以清理 N 个旧备份"
- `/yang-init` 重 init 时清理所有旧备份（既然要重 init，老备份意义不大）
- 用户手动 `rm .yang-state.json.backup-*` 永远 OK

不入版本控制：`.yang-state.json.backup-*` 应在 `.gitignore` 里（已含 `.yang-state.json` 通配规则）。
