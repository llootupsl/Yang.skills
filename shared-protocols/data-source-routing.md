...
被 yang-seed / yang-trends 引用。规定**何时**调热点工具、**调哪个**、**不调时怎么办**。
...
设计目的：保护 yang-seed 的核心论点——"好内容来自用户的真实经历，AI 不凭空 brainstorm"——同时不让"完全没想法"的新博主卡死。

---

## 触发矩阵（被 yang-seed Phase 1 引用）

| yang-seed Mode | 默认调？ | 触发条件 |
|---|---|---|
...
| `/yang-trends` 显式 | ✅ 调 | 主入口，无需解释 |
| `/yang-recommend` | ❌ 默认不调 | 已有 pool；除非 pool >7 天没更新 → 提示先 trends |
...
`.yang-state.json` 的 `enabled_trend_sources` 字段是**显式开关**：
...
yang-trends 显式调用时支持 override：`/yang-trends — sources: aihot`（仅这次用 aihot）。
...
**关键纪律**：所有失败都**不抛异常**。yang-seed 永远能跑——区别只是有没有外部素材。
...
```
[trend tool] → items
  → 去重（vs candidates.md / predictions/ / .yang-cache/trends-history.jsonl）
  → 粗打分（yang-seed 内联 rubric）
  → 写入 candidates.md（带 source 字段标明来自哪个 adapter）
```

yang-seed Mode C 拿到数据后**不**直接进 brainstorm，先入 candidates.md，再让 Claude 从池子里选。这样数据可追溯、可被后续 yang-recommend 复用。
...
3. 不需要改 yang-seed 内部逻辑——按 `enabled_trend_sources` 自动启用
4. CHANGELOG 标 MINOR

不要把硬编码"aihot"/"trendradar-mcp" 写进 yang-seed SKILL.md——保持 adapter 模型可扩展。
