...
被这些子 skill 引用：`yang-predict`、`yang-retro`、`templates/prediction.template.md`。
...
本协议的设计经过大量实际视频预测校准验证，rubric 权重基于实测数据持续迭代。
...
- `Script Hash` 必填——yang-shoot 时再 hash `videos/<id>/script.md`，不一致 → 复盘段加 integrity warning
...
- **`BlindScored By` 必填**——本次维度分由谁打：
  - `subagent-v1`：通过 Task tool 调 yang-score-blind sub-agent 拿到的盲打分（默认，Phase 2 路径）
...
> 如果是用户从零写的（没用 yang-seed），这一段写"用户原创稿，无 Claude 草稿对照"。
...
## 预测 v2 (replaces v1)        ← (可选) 拍后改稿 ≥30% 时由 yang-shoot 触发，append 不覆盖
...
- **新建文件**：yang-predict 写 `## 预测 v1`（不再裸 `## 预测`——为 v2 留 schema 一致性）
- **legacy 兼容**：v0.1.0 时期写的 `## 预测` 文件不动；hook 与 yang-retro 都识别
- **v2 触发条件**：yang-shoot 检测拍摄稿与 `scripts/<id>.md` 的 line-diff ≥ 30%（[V2_TRIGGER_THRESHOLD](../skills/yang-shoot/SKILL.md)），调用 `/yang-predict — mode: v2 — prediction-file: <path>`
- **append 而非覆盖**：v2 段插在 `## 复盘` 之前。v1 段**绝不**修改（hook 物理强制）
- **校准用谁**：yang-retro 读最后一个 `## 预测 vN` 算偏差；v1 留作历史档案
...
yang-retro 用此字段在 score-curve / bump 校准时区分两条数据线，避免混样。

---

## 子 skill 验收标准

`yang-predict` 写完一份预测后，必须自检 7 个组件齐全：
- 组件 5 / 7 在校准样本不足时仍写"N/A 段 + 解释"，**不允许直接跳过**
- header 的 `Calibration Samples` + `Confidence` 必填——读者一眼看到这次预测多可信

`yang-retro` 写复盘段时，必须先校验该文件的 7 个组件：
